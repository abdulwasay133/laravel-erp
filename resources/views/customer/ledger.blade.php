...
<div class="card">
    <div class="card-header">
        ...
        <p class="card-subtitle">Select a customer and date range to view transactions</p>
    </div>
    <div class="card-body">
            ...
            <div class="col-md-4">
                ...
                <select id="customerSelect" name="customer_id" class="form-select">
                    ...
                    @foreach($customers as $c)
                        <option value="{{ $c->id }}">{{ $c->first_name }} {{ $c->last_name }} @if($c->company) ({{ $c->company }}) @endif</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
...
$(function () {
    ...
    const table = $('#ledgerTable').DataTable({
        ...
        deferLoading: 0,
        ajax: {
            url  : '{{ route('customers.ledger.search') }}',
            type : 'POST',
            data: function (d) {
    ...
    $('#searchBtn').on('click', function (e) {
        e.preventDefault();
        if (!$('#customerSelect').val() || !$('#startDate').val() || !$('#endDate').val()) {
    ...
    $('#printBtn').on('click', function () {
        ...
        const url = '{{ route('customers.ledger.print') }}' + '?customer_id=' + customer_id + '&start_date=' + start + '&end_date=' + end;
